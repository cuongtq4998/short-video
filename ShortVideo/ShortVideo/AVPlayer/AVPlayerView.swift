...
//Custom Delegate for progress, playback status update callback
...
extension AVPlayerView: URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        if error == nil {
            WebCacheManager.shared().storeDataToDiskCache(data: self.data, key: self.cacheFileKey ?? "", exten: "m3u8")
        } else {
